...
class Wordsmith
  module Generate
    ...
    def to_html
      ...
      cmd = "pandoc -f markdown_mmd -S -s --toc -o #{File.join(output, "index.html")} -t html || pandoc -f markdown_mmd-smart -s --toc -o #{File.join(output, "index.html")} -t html"
    ...
    def to_odt
      info "Generating Openoffice ODT Manuscript format..."

      compile_stylesheets
      copy_assets

       cmd = "pandoc -f markdown_mmd -s -o #{File.join(output, "#{name}.odt")} -t odt"
       cmd += " \\\n#{files}"
     end
    ...
    def to_epub
      ...
      cmd = "pandoc -f markdown_mmd -S -o #{output}.epub -t epub || pandoc -f markdown_mmd-smart -o #{output}.epub -t epub"
      cmd += " \\\n--epub-metadata=#{metadata}" if metadata
      cmd += " \\\n--epub-cover-image=#{cover}" if cover
      cmd += " \\\n--css=#{epub_stylesheet}" if epub_stylesheet
      cmd += " \\\n#{files}"
      cmd
    end
